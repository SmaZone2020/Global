...
export default function AnalysisProgress({ steps }: AnalysisProgressProps) {
  return (
    <div className="flex flex-col items-center justify-center min-h-[50vh]">
      ...
      <p className="text-snow/50 text-sm mb-10">请稍候，分析可能需要 1-3 分钟</p>
      ...
      <div className="w-full max-w-md space-y-4">
        {steps.map((step, index) => (
          ...
          >
            ...
            {step.status === 'pending' && (
              <div className="w-8 h-8 rounded-full bg-ink-lighter flex items-center justify-center">
                <Circle className="w-4 h-4 text-snow/30" />
              </div>
            )}
            <span
      ...
      <p className="text-snow/30 text-xs mt-10">
        分析完成后将自动显示结果
      </p>
    </div>
  )
