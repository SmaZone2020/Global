...
export default function MarketTab({ content, onConfirmMarket, marketConfirmed }: MarketTabProps) {
  ...
  const compliance = content.complianceRisks ?? []
  const pricing = content.pricingSuggestion
  ...
  return (
    <div className="space-y-8">
      <section>
        ...
        {!marketConfirmed && candidates.length > 0 && (
            ...
            {personas.map((p, i) => (
              <div key={i} className="bg-cream-dark/40 border border-ink/8 p-5">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {(p.scenarios ?? []).map((s) => (
                    <span key={s} className="px-2 py-0.5 bg-cream-dark text-ink/50 text-xs">{s}</span>
                  ))}
                </div>
              </div>
            ))}
              ...
              <div key={i} className="bg-cream-dark/40 border border-ink/8 p-5">
                ...
                <div className="flex flex-wrap gap-1">
                  {(comp.channels ?? []).map((ch) => (
                    <span key={ch} className="px-2 py-0.5 bg-cream-dark text-ink/50 text-xs">{ch}</span>
                  ))}
                </div>
              </div>
            ))}
      ...
      {compliance.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {compliance.map((cr, i) => (
              <div key={i} className="bg-unverified/5 border border-unverified/20 p-4">
